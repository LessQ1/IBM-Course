...
dropdown_style = {'width': '25%', 'padding': '3px', 'fontSize': '20px', 'text-align': 'center'}

# Create the layout of the app
app.layout = html.Div([
    # Add title to the dashboard
    html.H1("Automobile Sales Statistics", style={'text-align': 'center', 'color': '#503D36', 'fontSize': 24}),
...
def update_output_container(selected_statistics, input_year):
    if selected_statistics == 'Recession Period Statistics':
        ...
        bar_data = recession_data.groupby(['unemployment_rate','Vehicle_Type'])['Automobile_Sales'].mean().reset_index()
        R_chart4 = dcc.Graph(
        ...
        return [
                ...
                html.Div(className='chart-grid', children=[html.Div(children=R_chart1),html.Div(children=R_chart2)],style={'display': 'flex', 'flex-direction': 'column'}),
                html.Div(className='chart-grid', children=[html.Div(children=R_chart3),html.Div(children=R_chart4)],style={'display': 'flex', 'flex-direction': 'column'})
            ]
        ...
        month_sale = yearly_data.groupby('Month')['Automobile_Sales'].sum().reset_index()
        Y_chart2 = dcc.Graph(
            figure=px.line(month_sale, 
                x='Month', 
                y='Automobile_Sales', 
        ...
        return [
                ...
                html.Div(className='chart-grid', children=[html.Div(children=Y_chart1),html.Div(children=Y_chart2)],style={'display': 'flex', 'flex-direction': 'column'}),
                html.Div(className='chart-grid', children=[html.Div(children=Y_chart3),html.Div(children=Y_chart4)],style={'display': 'flex', 'flex-direction': 'column'})
            ]
    else:       
        return None
